...
- Wait for the exact trigger command `Go!` before generating widgets, screens, services, routes, or backend code.
...
- Prefer this structure for new or clean projects:
  ...
  - `lib/features/`
  - `lib/shared/widgets/`
  - `lib/shared/providers/` only when app-wide providers are truly needed
...
- Use route guards for auth-sensitive or onboarding-sensitive flows.
...
- Prefer `const` constructors where meaningful.
- Use lazy list builders for long data lists.
- Avoid rebuilding large widget subtrees unnecessarily.
- Keep image usage efficient.
- Follow obvious Flutter performance basics without premature optimization.
