---
name: flutter-protocol
description: Enforce a strict Flutter development protocol with a gated "Go!" execution flow. Use when Codex must analyze or implement Flutter app work under this exact protocol, especially for Material 3 apps, go_router routing, Riverpod state management, feature-first project structure, responsive mobile UI, Dart null safety, forms, loading states, and maintainable Flutter architecture without backend-specific assumptions.
---
...
- Before `Go!`, do not:
  ...
  - write backend or database code
...
- Prefer native Flutter `Form`, `TextFormField`, and validators for forms unless the project already uses another consistent form package.
...
- Use:
  ...
  - `try/catch` for async and repository error handling
...
- Always show a visible loading state during submit, save, refresh, delete, sign-in, or button-triggered async actions.
...
- Always include:
  - file path
  - typed Dart code
  - clear widget, provider, or service responsibilities
